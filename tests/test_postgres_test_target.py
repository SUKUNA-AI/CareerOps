...
@pytest.mark.parametrize(
    "dsn",
    [
        "postgresql://user:pass@localhost:5432/careerops_v2_ci",
        "postgresql://user:pass@127.0.0.1:5432/careerops_v2_test",
        "host=localhost hostaddr=127.0.0.1 dbname=careerops_v2_testing user=user",
    ],
)
def test_postgres_test_target_accepts_explicit_local_disposable_dsn(dsn: str) -> None:
    target = validate_postgres_test_dsn(dsn)

    assert target.database.startswith("careerops_v2_")
...
@pytest.mark.parametrize(
    "dsn",
    [
        "postgresql://user:pass@db.example.com:5432/careerops_v2_ci",
        "host=localhost hostaddr=203.0.113.10 dbname=careerops_v2_ci user=user",
        "postgresql://user:pass@localhost:5432/postgres",
        "postgresql://user:pass@localhost:5432/careerops_v2",
        "postgresql://user:pass@localhost:5432/careerops_prod_test",
        "host=localhost dbname=careerops_v2_ci port=not-a-number",
        "service=careerops_ci host=localhost dbname=careerops_v2_ci",
    ],
)
