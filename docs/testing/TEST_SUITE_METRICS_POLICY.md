...
Measured locally on 2026-09-06 from branch
`CAR-139-test-suite-architecture-cleanup` after CAR-145, using the worktree-local
Python 3.13 virtual environment:

| Measurement | Value | Evidence |
|---|---:|---|
| Total collected cases | 213 | Main `tests/` suite |
| FAST cases | 204 | `-m "not integration_postgres"` |
| PostgreSQL integration cases | 9 | Collected, not executed |
| FAST elapsed time | 2.03 s | One warm local run on Windows |
...
- branch coverage percentage when reported;
...
Before CAR-146 is considered complete, record:

- 213 total collected cases;
- 204 FAST passing cases;
- 9 PostgreSQL integration cases collected;
- three comparable FAST duration runs and their median;
- one FAST branch-coverage measurement generated by the canonical command;
- Ruff, mypy, and `git diff --check` passing.
