...
_LIVE_PRE_SUBMIT_STATUSES = "'preparing','precheck'"
_UNRESOLVED_SUBMIT_STATUSES = (
    "'submitting','submitted_unconfirmed','uncertain','reconciliation_required'"
)
        ...
        SELECT
            NOT EXISTS (
                ...
                WHERE unresolved.account_id = %(account_id)s
                  AND unresolved.id IS DISTINCT FROM %(exclude_application_id)s
    ...
    cursor = await connection.execute(
        ...
        WHERE ac.status = 'eligible'
          ...
          AND NOT EXISTS (
              SELECT 1
              FROM careerops_v2.applications unresolved
              WHERE unresolved.account_id = acc.id
                AND unresolved.status IN ({_UNRESOLVED_SUBMIT_STATUSES})
          )
          AND NOT EXISTS (
              SELECT 1
