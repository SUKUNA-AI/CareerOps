...
class ApplicationServiceSettings:
    ...
    transport_operation_timeout_seconds: float = 90.0
    ...
    def from_env(cls) -> ApplicationServiceSettings:
        ...
        operation_timeout_seconds = _float_env(
            "CAREEROPS_APPLICATION_TRANSPORT_OPERATION_TIMEOUT_SECONDS",
            90.0,
        )
        if transport_timeout_seconds > operation_timeout_seconds:
            raise ValueError(
                "CAREEROPS_APPLICATION_TRANSPORT_TIMEOUT_SECONDS must not exceed "
                "CAREEROPS_APPLICATION_TRANSPORT_OPERATION_TIMEOUT_SECONDS"
            )
        if operation_timeout_seconds >= lease_seconds:
            raise ValueError(
                "CAREEROPS_APPLICATION_TRANSPORT_OPERATION_TIMEOUT_SECONDS must be smaller than "
        ...
        return cls(
            ...
            transport_operation_timeout_seconds=operation_timeout_seconds,
